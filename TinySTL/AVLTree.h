...
private:
    ...
    void print_preorder_aux(const string& delim, std::ostream& os,
                            const node* ptr)const;
    void print_inorder_aux(const string& delim, std::ostream& os,
                           const node* ptr)const;
    void print_postorder_aux(const string& delim, std::ostream& os,
                             const node* ptr)const;
};// end of avl_tree
namespace Detail
{
...
private:
    ...
    typedef typename avl_tree<typename T::value_type>::const_reference
    const_reference;
...
private:
    ...
    std::set<const T*>
    visited_; //当前节点是否被访问过（此node被访问说明其左子树已被访问了）
public:
    ...
    avl_iter left()const
    {
        return avl_iter(ptr_->left_, container_);
    }
    avl_iter right()const
    {
        return avl_iter(ptr_->right_, container_);
    }
