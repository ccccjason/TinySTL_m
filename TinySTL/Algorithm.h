               ...
               Compare comp) //1.[first, last], 2.headr points the header of the heap
{
    if (first != last) {
        ...
        for (auto cur = last; parentIndex >= 0 &&
             cur != head; parentIndex = (index - 1) / 2) {
                 ...
                 Compare comp) //1.[first, last], 2.headr points the header of the heap
{
    if (first != last) {
        ...
        for (auto cur = first; leftChildIndex < (last - head + 1) &&
             cur < last; leftChildIndex = index * 2 + 1) {
...
{
    return TinySTL::equal(first1, last1, first2,
                          TinySTL::equal_to<typename TinySTL::iterator_traits<InputIterator1>::value_type>());
}
template <class InputIterator1, class InputIterator2, class BinaryPredicate>
bool equal(InputIterator1 first1, InputIterator1 last1,
           InputIterator2 first2, BinaryPredicate pred)
