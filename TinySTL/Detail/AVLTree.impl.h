...
void avl_tree<T>::erase_elem(const T& val, node*& p)
{
    ...
    if (p->data_ != val) {
        ...
        if (p->left_ != 0 && p->right_ != 0) { // has two children
            ...
            //随机选择删除左右，使得删除操作更平衡
            auto pos = (choose == 0 ?
                        const_cast<node*>(find_min_aux(p->right_).ptr_) : const_cast<node*>
                        (find_max_aux(p->left_).ptr_));
            p->data_ = pos->data_;
            return (choose == 0 ? erase_elem(pos->data_, p->right_) : erase_elem(pos->data_,
                    p->left_));
...
template<class T>
void avl_tree<T>::print_inorder_aux(const string& delim, std::ostream& os,
                                    const node* ptr)const
{
    if (ptr != 0) {
...
template<class T>
void avl_tree<T>::print_postorder_aux(const string& delim, std::ostream& os,
                                      const node* ptr)const
{
    if (ptr != 0) {
...
template<class T>
typename avl_tree<T>::const_iterator avl_tree<T>::find_aux(const T& val,
        const node* ptr)const
{
    while (ptr != 0) {
        if (ptr->data_ < val) {
...
template<class T>
typename avl_tree<T>::const_iterator avl_tree<T>::find_max_aux(
    const node* ptr)const
{
...
template<class T>
typename avl_tree<T>::const_iterator avl_tree<T>::find_min_aux(
    const node* ptr)const
{
