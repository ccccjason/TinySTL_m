...
void binary_search_tree<T>::erase_elem(const T& val, node*& ptr)
{
    ...
    if (ptr->data_ != val) {
        ...
        if (ptr->left_ != 0 && ptr->right_ != 0) { // has two children
            ...
            //随机选择删除左右，使得删除操作更平衡
            auto pos = (choose == 0 ?
                        const_cast<node*>(find_min_aux(ptr->right_).ptr_) : const_cast<node*>
                        (find_max_aux(ptr->left_).ptr_));
            ptr->data_ = pos->data_;
            return (choose == 0 ? erase_elem(pos->data_,
                                             ptr->right_) : erase_elem(pos->data_, ptr->left_));
...
template<class T>
void binary_search_tree<T>::insert_elem(const T& val,
                                        node*& ptr) //重复的元素不插入
{
    if (ptr == 0) {
        ...
        std::ostream& os)const
{
    ...
    if (temp != 0) {
        ...
        while (!q.empty()) {
            ...
            if (temp->left_ != 0) {
                q.push_back(temp->left_);
            }

            if (temp->right_ != 0) {
                q.push_back(temp->right_);
            }
...
template<class T>
typename binary_search_tree<T>::const_iterator
binary_search_tree<T>::find_min_aux(const node* ptr)const
{
...
template<class T>
typename binary_search_tree<T>::const_iterator
binary_search_tree<T>::find_max_aux(const node* ptr)const
{
...
template<class T>
typename binary_search_tree<T>::const_iterator binary_search_tree<T>::find_aux(
    const T& val, const node* ptr)const
{
    while (ptr) {
        if (val == ptr->data_) {
            break;
...
template<class T>
typename binary_search_tree<T>::const_iterator binary_search_tree<T>::find(
    const T& val)const
{
