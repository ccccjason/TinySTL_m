...
template<class T>
typename list<T>::const_iterator list<T>::changeIteratorToConstIterator(
    iterator& it)const
{
    using nodeP = Detail::node<const T>* ;
    auto temp = (list<const T>* const)this;
    auto ptr = it.p;
    Detail::node<const T> node(ptr->data, (nodeP)(ptr->prev), (nodeP)(ptr->next),
                               temp);
...
bool operator== (const list<T>& lhs, const list<T>& rhs)
{
    ...
    for (; node1 != lhs.tail.p &&
         node2 != rhs.tail.p; node1 = node1->next, node2 = node2->next) {
