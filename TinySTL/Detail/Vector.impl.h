...
template<class T, class Alloc>
void vector<T, Alloc>::reallocateAndFillN(iterator position, const size_type& n,
...
void vector<T, Alloc>::insert_aux(iterator position,
                                  ...
                                  std::false_type)
{
    difference_type locationLeft = endOfStorage_ -
                                   finish_; // the size of left storage
    ...
    if (locationLeft >= locationNeed) {
        if (finish_ - position > locationNeed) {
            ...
            iterator temp = TinySTL::uninitialized_copy(first + (finish_ - position), last,
                            finish_);
...
template<class T, class Alloc>
typename vector<T, Alloc>::size_type vector<T, Alloc>::getNewCapacity(
    size_type len)const
{
