...
public:
    ...
    array_type suffixArray()const
    {
        return _suffix_array;
    }
    array_type heightArray()const
    {
        return _height_array;
    }
    array_type rankArray()const
    {
        return _rank_array;
    }
private:
...
template<class InputIterator>
void suffix_array::calSuffix(const InputIterator arr, size_t len,
                             size_t max_len)
{
    //采用了罗穗骞论文中实现的倍增算法
    //算法时间复杂度 = O(nlg(n))
    ...
    for (j = 1, p = 1; p < len; j *= 2, max_len = p) {
        ...
        for (i = 0; i < len; i++) if (_suffix_array[i] >= j) {
                y[p++] = _suffix_array[i] - j;
            }
        ...
        for (t = x, x = y, y = t, p = 1, x[_suffix_array[0]] = 0, i = 1; i < len; i++) {
            x[_suffix_array[i]] = cmp(y, _suffix_array[i - 1], _suffix_array[i],
                                      j) ? p - 1 : p++;
        }
    }
}
