...
void testAllOf()
{
    ...
    assert(TinySTL::all_of(foo.begin(), foo.end(), [](int i) {
        return i % 2;
    }));
}
void testNoneOf()
{
    ...
    assert(TinySTL::none_of(foo.begin(), foo.end(), [](int i) {
        return i < 0;
    }));
}
void testAnyOf()
{
    ...
    assert(std::any_of(foo.begin(), foo.end(), [](int i) {
        return i < 0;
    }));
}
void testForEach()
...
void testFind()
{
    ...
    assert(TinySTL::find_if(v.begin(), v.end(), [](int i) {
        return i < 0;
    }) == v.end());
    assert(TinySTL::find_if_not(v.begin(), v.end(), [](int i) {
        return i < 0;
    }) != v.end());
}
void testFindEnd()
{
    ...
    int needle2[] = { 4, 5, 1 };
    it = TinySTL::find_end(v.begin(), v.end(), needle2, needle2 + 3, [](int i,
    int j) {
        return i == j;
    });
    assert(it == v.begin() + 3);
...
void testAdjacentFind()
{
    ...
    it = TinySTL::adjacent_find(++it, v.end(), [](int i, int j) {
        return i == j;
    });
    assert(*it == 10);
}
void testCount()
{
    ...
    mycount = TinySTL::count_if(myints, myints + 8, [](int i) {
        return i % 2 == 0;
    });
    assert(mycount == 8);
}
void testMismatch()
{
    ...
    ++mypair.first;
    ++mypair.second;
    mypair = TinySTL::mismatch(mypair.first, v.end(), mypair.second, [](int i,
    int j) {
        return i == j;
    });
}
void testEqual()
{
    ...
    v[3] = 81;
    assert(!TinySTL::equal(v.begin(), v.end(), myints, [](int i, int j) {
        return i == j;
    }));
}
...
void testSearch()
{
    ...
    it = std::search(v.begin(), v.end(), needle2, needle2 + 3, [](int i, int j) {
        return i == j;
    });
    assert(it == v.end());
}
